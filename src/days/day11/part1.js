...
function main() {
  ...
  const { galaxies } = expandCosmosWithGalaxy(cosmos);

  const galaxyPairs = computeGalaxyPairs(galaxies);

  const distances = galaxyPairs.map(([gId1, gId2]) =>
    computeDistance(galaxies[gId1].position, galaxies[gId2].position)
  );
