...
function main() {
  ...
  // done in part1: result: 7107
  const maxDistance = findFarthestDistance(lines, startPosition);
  // const maxDistance = 7107;
  const vertices = gatherVertices(lines, startPosition);
  const area = computeArea(vertices);
  const areaWithin = computeAreaWithin(area, maxDistance);
  console.debug(`area within: ${areaWithin}`);
}
