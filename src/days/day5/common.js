 ...
 *
 * @param {string[]} lines
 * @returns {{ seedsRanges: number[], inputMaps: InputMaps }}
 */
  ...
  for (const line of lines) {
    switch (true) {
      ...
      case line.startsWith('seeds: '): {
        seedsRanges = strToNbArr(line.substring(7));
        break;
      }
 ...
 */
export function computeOneSeedLocation(seed, inputMaps) {
  let nextStepNb = seed;
 ...
 */
export function computeMinSeedLocation(seedsRanges, inputMaps) {
  let seed = -1;
